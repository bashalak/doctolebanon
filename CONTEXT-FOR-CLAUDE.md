...
- **NEXT = Part 2: Legal info (modal, placeholder text) + Delete account.** Delete caveat: client can delete the user's DATA (profile/appointments/relatives/reviews/owned doctors) + sign out, but NOT the auth.users record (needs Edge Function w/ service_role — Stage E). Will flag in UI.
