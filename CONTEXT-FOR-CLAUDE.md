...
- **v0.20 (building): bulk/recurring availability generator** — doctor picks From/To dates, weekdays, Start/End time, interval → generateSlots() builds rows client-side + `sb.from('slots').upsert(rows,{onConflict:'doctor_id,slot_date,slot_time',ignoreDuplicates:true})`. (User asked: filling slots one-by-one is impractical, wants e.g. next 3 months at once.)
