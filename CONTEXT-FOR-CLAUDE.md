...
## ACCOUNT MENU + PROFILE (v0.34–v0.35)
- v0.34: patient profile availability shown ONE day at a time — `slotDayIdx` + slotPrevDay/slotNextDay; openDoctor slots query `.limit(300)`; reschedule `.limit(120)`. (Replaced earlier show-more-dates.)
- **v0.35: account dropdown menu + Profile.** Header decluttered: removed navDash/navAdmin/My-appointments buttons (and their applyLang lines + navApptLabel). `#authArea` now renders an account menu: `email ▾` button → `.menu-panel` dropdown (Profile/My appointments/Dashboard if doctor/Admin if admin/Log out). toggleMenu()/closeMenu() + document outside-click closes it. **Profile is a MODAL** (showProfile): `profiles` table (id=auth.uid, name/address/phone, RLS own) — saveProfileInfo() upsert; changeEmail()/changePassword() via `sb.auth.updateUser`. apptBadge now lives inside the menu item; updateBadge called at end of renderAuthUI. i18n profile/changeEmail/changePassword/newPassword/saved. Footer v0.35.
- **Part 2 DONE (v0.36–v0.37):** showLegal() modal (i18n legalBody placeholder, mentions Law 81). deleteAccount() (now a button INSIDE the Profile modal, not the menu) — best-effort deletes appointments/reviews/relatives/owned doctors/profile by uid + signOut + closeModal. SQL added: `owner deletes own doctor` policy. CAVEAT (flagged to user + in legalBody): auth.users record NOT deleted from client — needs Edge Function w/ service_role (Stage E). i18n legal/legalBody/deleteAccount/deleteConfirm/deleteDone. Footer v0.37. D11 COMPLETE.

## D6 BOOK FOR FAMILY (done, v0.30–v0.32)
- `relatives` table (id, user_id default auth.uid(), name, relation) + RLS own select/insert/delete. JS: myRelatives global, loadRelatives() (called in refreshRoles). After redesign (user disliked family section in My-appointments): family mgmt is now IN THE BOOKING FORM. pickSlot form has "Who is this for?" `#bFor` select (Myself / saved relatives / "Someone else…") → onForChange() fills #bName, shows "save to my family" checkbox for new, shows "✕ remove from my family" link for a saved one → removeSelectedRelative() deletes + rebuilds form. confirmBooking saves a new person if "save" ticked. i18n forWhom/forMyself/addSomeone/saveToFamily/removeRel (old familyTitle/relName/etc now unused). Footer v0.32.
